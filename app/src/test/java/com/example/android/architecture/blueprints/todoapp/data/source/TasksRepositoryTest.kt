...
class TasksRepositoryTest {
    ...
    @Test fun getTasks_repositoryCachesAfterFirstApiCall() = runBlockingSilent {
    ...
    @Test fun getTasks_requestsAllTasksFromLocalDataSource() = runBlockingSilent {
    ...
    @Test fun saveTask_savesTaskToServiceAPI() = runBlockingSilent {
    ...
    @Test fun completeTask_completesTaskToServiceAPIUpdatesCache() = runBlockingSilent {
    ...
    @Test fun completeTaskId_completesTaskToServiceAPIUpdatesCache() = runBlockingSilent {
        // Given a stub active task with title and description added in the repository
        val newTask = Task(TASK_TITLE_1, TASK_GENERIC_DESCRIPTION)
        with(tasksRepository) {
            saveTask(newTask)
    ...
    @Test fun activateTask_activatesTaskToServiceAPIUpdatesCache() = runBlockingSilent {
    ...
    @Test fun activateTaskId_activatesTaskToServiceAPIUpdatesCache() = runBlockingSilent {
        // Given a stub completed task with title and description in the repository
        val newTask = Task(TASK_TITLE_1, TASK_GENERIC_DESCRIPTION).apply { isCompleted = true }
        with(tasksRepository) {
            saveTask(newTask)
    ...
    @Test fun getTask_requestsSingleTaskFromLocalDataSource() = runBlockingSilent {
    ...
    @Test fun deleteCompletedTasks_deleteCompletedTasksToServiceAPIUpdatesCache() = runBlockingSilent {
        with(tasksRepository) {
    ...
    @Test fun deleteAllTasks_deleteTasksToServiceAPIUpdatesCache() = runBlockingSilent {
        with(tasksRepository) {
            // Given 2 stub completed tasks and 1 stub active tasks in the repository
            val newTask = Task(TASK_TITLE_1, TASK_GENERIC_DESCRIPTION).apply { isCompleted = true }
            saveTask(newTask)
    ...
    @Test fun deleteTask_deleteTaskToServiceAPIRemovedFromCache()  = runBlockingSilent {
        with(tasksRepository) {
    ...
    @Test fun getTasksWithDirtyCache_tasksAreRetrievedFromRemote()  = runBlockingSilent {
    ...
    @Test fun getTasksWithLocalDataSourceUnavailable_tasksAreRetrievedFromRemote() = runBlockingSilent {
    ...
    @Test fun getTasksWithBothDataSourcesUnavailable_firesOnDataUnavailable() = runBlockingSilent {
        // And the local data source has no data available
        setTasksNotAvailable(tasksLocalDataSource)
    ...
    @Test fun getTaskWithBothDataSourcesUnavailable_firesOnDataUnavailable() = runBlockingSilent {
        // Given a task id
        val taskId = "123"
    ...
    @Test fun getTasks_refreshesLocalDataSource() = runBlockingSilent {
    ...
    private fun twoTasksLoadCallsToRepository() = runBlockingSilent {
        ...
        `when`(tasksLocalDataSource.getTasks()).thenReturn(Result.Error(RemoteDataNotFoundException()))
    ...
    private fun setTasksNotAvailable(dataSource: TasksDataSource) = runBlockingSilent {
        `when`(dataSource.getTasks()).thenReturn(Result.Error(RemoteDataNotFoundException()))
    }
    ...
    private fun setTaskNotAvailable(dataSource: TasksDataSource, taskId: String) = runBlockingSilent {
        `when`(dataSource.getTask(taskId)).thenReturn(Result.Error(RemoteDataNotFoundException()))
    }
