...
class TasksRepository(
    ...
    override suspend fun getTasks(): Result<List<Task>> {
        ...
        if (cacheIsDirty) {
            ...
            val localTask = tasksLocalDataSource.getTasks()

            return when(localTask) {
                is Result.Success -> {
                    refreshCache(localTask.data)
                    Result.Success(localTask.data)
                }
                is Result.Error -> {
                    val remoteTask = tasksRemoteDataSource.getTasks()
                    when (remoteTask) {
                        is Result.Success -> Result.Success(remoteTask.data)
    ...
    override suspend fun getTask(taskId: String): Result<Task> {
        ...
        return when(localTask) {
            is Result.Success -> Result.Success(localTask.data)
            is Result.Error -> {
    ...
    private suspend fun getTasksFromRemoteDataSource():Result<List<Task>> {
        val taskList = tasksRemoteDataSource.getTasks()

        return when (taskList) {
            is Result.Success -> {
                refreshCache(taskList.data)
                refreshLocalDataSource(taskList.data)
                Result.Success(taskList.data)
            }
            is Result.Error -> Result.Error(RemoteDataNotFoundException())
        }
    }
