...
class TasksLocalDataSource private constructor(
    ...
    override suspend fun getTasks(): Result<List<Task>> = withContext(appExecutors.ioContext) {
        val tasksList = tasksDao.getTasks()
        if (tasksList != null) {
    ...
    override suspend fun clearCompletedTasks() = withContext(appExecutors.ioContext) {
        val value = tasksDao.deleteCompletedTasks()
    }
    ...
    override suspend fun deleteTask(taskId: String) = withContext(appExecutors.ioContext) {
        val value = tasksDao.deleteTaskById(taskId)
    }
